...
<head>
      ...
      tailwind.config = {
        theme: {
          extend: {
            ...
            colors: { brand: { 50:'#f0f4ff',100:'#e0eaff',200:'#c2d4ff',500:'#4f6ef7',600:'#3b55e8',700:'#2a3fcc' } },
            keyframes: {
              'slide-in':  { '0%':{ transform:'translateX(110%)', opacity:'0' }, '100%':{ transform:'translateX(0)', opacity:'1' } },
              'slide-out': { '0%':{ transform:'translateX(0)', opacity:'1' }, '100%':{ transform:'translateX(110%)', opacity:'0' } },
              'pulse-red': { '0%,100%':{ boxShadow:'0 0 0 0 rgba(239,68,68,0.4)' }, '50%':{ boxShadow:'0 0 0 10px rgba(239,68,68,0)' } },
    ...
    <style>
      ...
      .nav-link.active   { background:rgba(79,110,247,.12); color:#4f6ef7; }
      .nav-link.active svg { color:#4f6ef7; }
      .ticket-row:hover  { background:#f8faff; }
      .sla-bell-active   { animation:pulse-red 1.5s ease-in-out infinite; }
      .sla-row-critical  { border-left:3px solid #ef4444; }
      .sla-row-high      { border-left:3px solid #f59e0b; }
      .sla-row-medium    { border-left:3px solid #4f6ef7; }
      .sla-row-low       { border-left:3px solid #22c55e; }
    </style>
    @stack('head')
</head>
...
<div class="flex h-screen overflow-hidden">
    ...
    <aside class="flex flex-col w-60 bg-white border-r border-gray-100 flex-shrink-0 overflow-y-auto">
        ...
        <nav class="flex-1 px-3 py-4 space-y-0.5">
            ...
            {{-- Tickets section --}}
            <div class="pb-1 px-3">
                <p class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Helpdesk</p>
            </div>
            
            <a href="{{ route('admin.dashboard') }}"
               class="nav-link flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-gray-700 font-medium hover:bg-gray-50 {{ $path === 'admin' ? 'active' : '' }}">
            ...
            <a href="{{ route('tickets.create') }}"
               class="nav-link flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-gray-700 font-medium hover:bg-gray-50 {{ str_contains($path,'create') && str_contains($path,'ticket') ? 'active' : '' }}">
            ...
            <a href="{{ route('assets.index') }}"
               class="nav-link flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-gray-700 font-medium hover:bg-gray-50 {{ str_starts_with($path,'assets') && !str_contains($path,'create') ? 'active' : '' }}">
                <svg class="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                All Assets
            </a>
            ...
            <a href="{{ route('users.index') }}"
               class="nav-link flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-gray-700 font-medium hover:bg-gray-50 {{ str_contains($path,'admin/users') ? 'active' : '' }}">
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <div class="mx-6 mt-4 flex items-center gap-2 p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-800">
            <svg class="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 8v4m0 4h.01"/></svg>
            {{ session('error') }}
        </div>
        @endif
