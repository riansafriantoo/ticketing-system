        ...
        <div class="bg-white rounded-xl border border-gray-100 p-5">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="category" class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-brand-400 bg-white">
                        @foreach($categories as $c)
                        <option value="{{ $c->value }}" {{ old('category', $ticket->category->value) === $c->value ? 'selected' : '' }}>
                            {{ $c->label() }}
                        </option>
                        @endforeach
                    </select>
